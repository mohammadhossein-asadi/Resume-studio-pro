...
export function ArchitectureTemplate({ resume }: TemplateProps) {
  ...
  const sidebarSections = sections
    .filter((s) => s.visible)
    .filter((s) => s.type === "skills" || s.type === "certifications");
  ...
  return (
    ...
    >
      {/* Left Sidebar */}
      <div className={`w-1/4 ${isRTL ? "pl-4" : "pr-4"} shrink-0`}>
        {/* Name - Vertical Stacked Style */}
        <div className="mb-5 pb-4" style={{ borderBottom: settings.showDividers ? `1px solid ${settings.primaryColor}30` : "none" }}>
          <div className="mb-1">
            <p className="text-[10px] uppercase tracking-[0.2em] text-gray-400 mb-0.5">{resume.language === "fa" ? "نام" : "First Name"}</p>
            <h1 className="text-lg font-bold tracking-wide" style={{ color: settings.primaryColor }}>
              {personalInfo.firstName}
            </h1>
          </div>
          <div className="mb-2">
            <p className="text-[10px] uppercase tracking-[0.2em] text-gray-400 mb-0.5">{resume.language === "fa" ? "نام خانوادگی" : "Last Name"}</p>
            <h1 className="text-lg font-bold tracking-wide" style={{ color: settings.primaryColor }}>
              {personalInfo.lastName}
            </h1>
          </div>
          {personalInfo.title && (
            <p className="text-[10px] text-gray-500 tracking-wide">{personalInfo.title}</p>
          )}
        </div>

        {/* Contact */}
        <div className="mb-5">
          <h2 className="text-[9px] font-bold uppercase tracking-[0.2em] mb-2" style={{ color: settings.primaryColor }}>
        ...
        {/* Sidebar Sections (certifications, skills) */}
      ...
      {/* Right Main Content */}
      <div className={`w-3/4 ${isRTL ? "pr-4" : "pl-4"} border-l`} style={{ borderColor: `${settings.primaryColor}15` }}>
        {/* Summary */}
