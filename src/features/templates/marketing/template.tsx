...
export function MarketingTemplate({ resume }: TemplateProps) {
  ...
  return (
    <div dir={dir} className="h-full p-8" style={{ fontFamily: '"Inter", sans-serif' }}>
      {/* Bold header */}
      <div className="mb-6">
        <h1 className="text-3xl font-black tracking-tight mb-1" style={{ color: settings.primaryColor }}>
          {personalInfo.firstName} {personalInfo.lastName}
        </h1>
        {personalInfo.title && (
          <p className="text-sm font-medium text-gray-600 mb-2">{personalInfo.title}</p>
        )}
      ...
      {personalInfo.summary && (
        <div className="mb-5 p-4 rounded-lg" style={{ backgroundColor: settings.primaryColor + "10" }}>
      ...
      {sections
        ...
        .map((section) => (
          <div key={section.id} className="mb-5">
            <h2 className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: settings.primaryColor }}>
